...
void parse_rttattr(struct rtattr *tb[], int max, struct rtattr *rta, int len) {
    ...
    while (1) {
        ...
        for (h = (struct nlmsghdr*)buf; status >= (ssize_t)sizeof(*h); ) {   // read all messagess headers
            ...
            if ((h->nlmsg_type == RTM_NEWROUTE) || (h->nlmsg_type == RTM_DELROUTE)) { // some changes in routing table
                ...
                parse_rttattr(tb, IFLA_MAX, IFLA_RTA(ifi), h->nlmsg_len);  // get attributes
                ...
                parse_rttattr(tba, IFA_MAX, IFA_RTA(ifa), h->nlmsg_len);
